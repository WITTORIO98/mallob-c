...
if echo $solvers|grep -q "c"; then
    if [ ! -d cadical ]; then
        if [ ! -f cadical.zip ]; then
            # for fixing a branch instead of a commit, prepend "refs/heads/"
            branchorcommit="refs/heads/develop"
            curl -L -o cadical.zip https://github.com/WITTORIO98/cadical/archive/${branchorcommit}.zip
            #wget -nc https://github.com/WITTORIO98/cadical/archive/${branchorcommit}.zip -O cadical.zip
        fi
        unzip cadical.zip
        mv cadical-* cadical
    fi
